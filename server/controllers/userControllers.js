...
const registerUser = async (req, res, next) => {
  try {
    ...
    res.status(201).json(`New user ${newUser.email} registered successfuly.`);
  } catch (error) {
...
const loginUser = async (req, res, next) => {
  try {
    ...
    if (!comparePass) {
      return next(new HttpError("Invaild password.", 422));
    }
...
const editUser = async (req, res, next) => {
  try {
    ...
    newInfo = await User.findByIdAndUpdate(
      req.user.id,
      { name, email},
      { new: true }
    );
