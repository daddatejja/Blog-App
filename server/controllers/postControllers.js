...
const createPost = async (req, res, next) => {
    try {
        ...
        if(!title || !category || !desc){
            return next(new HttpError("Fill in all fileds and choose a thumbnail.", 422))
        }

        const {thumbnail} = req.files;
        // check the file size
        if(thumbnail.size > 2000000){
            return next(new HttpError("Thumbnail size is too large. File should be less than 2mb."))
        }
